...
export function dbToProperty(row: DbProperty): Property {
  return {
    ...
    saleCategory: (row.saleCategory as Property["saleCategory"]) ?? "primary",
    downPaymentPercent: row.downPaymentPercent ?? undefined,
    installmentYears: row.installmentYears ?? undefined,
    price: row.price,
    area: row.area,
